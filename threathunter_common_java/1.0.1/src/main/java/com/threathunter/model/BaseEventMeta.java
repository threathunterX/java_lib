...
public class BaseEventMeta implements EventMeta {
    ...
    public VariableMeta getSrcVariable() {
        VariableMeta result = null;
        Identifier id = getSrcVariableID();
        if (id != null) {
            result = VariableMetaRegistry.getInstance().getVariableMeta(id);
        }
    ...
    public static BaseEventMeta from_json_object(Object obj) {
        ...
        result.derived = (Boolean)map.get("derived");
        ...
        if (propertiesObj != null) {
            for(Object o : propertiesObj) {
                result.properties.add(Property.from_json_object(o));
            }
        }
        result.expire = ((Number) map.get("expire")).longValue();
