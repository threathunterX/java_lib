...
public abstract class VariableMeta {
    ...
    public static void init() {
        String[] packages = CommonDynamicConfig.getInstance().getStringArray("nebula.variablemeta.packages");
        if (packages != null) {
     ...
     */
    public abstract List<Identifier> getSrcVariablesID();

    /**
     * Return the source variables if it is a derived variable
     */
    public abstract List<VariableMeta> getSrcVariables();

    /**
     * Return the source event identifier if it is a non-derived variable.
     */
    public abstract Identifier getSrcEventID();

    /**
     * Return the source event meta if it is a non-derived variable.
     */
    public abstract EventMeta getSrcEvent();
     ...
     *     <li>value     |  {@code NamedType.DOUBLE}  | {@code Variable.getValue()}</li>
     ...
     */
    public abstract List<PropertyReduction> getPropertyReductions();
    ...
    /**
     * whether the variable should maintain 20 keys with top values
     *
     * @return
     */
    public abstract boolean isTopValue();

    /**
     * whether the variable should maintain 20 values for each key
     *
     * @return
     */
    public abstract boolean isKeyTopValue();
