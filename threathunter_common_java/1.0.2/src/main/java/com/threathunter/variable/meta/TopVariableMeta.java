...
public class TopVariableMeta extends BaseVariableMeta {
    ...
    protected List<Property> parseProperties() {
        ...
        if (this.groupKeys != null && groupKeys.size() > 0) {
            this.getGroupKeys().forEach(property -> propertyList.add(new Property(this.getId(), property.getName(), property.getType())));
        }
        return propertyList;
    }
    ...
    public static TopVariableMeta from_json_object(final Object obj) {
        ...
        result.setGroupKeys(VariableMetaRegistry.getInstance().getVariableMeta(
                result.getSrcVariableMetasID().get(0)).getGroupKeys());
        return result;
    }
}
