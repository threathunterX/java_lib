...
public class BloomFilter<E> implements Serializable {
    ...
    private final AtomicLong numberOfAddedElements = new AtomicLong(); // number of elements actually added to the Bloom filter
    private final int k; // number of hash functions

    static final Charset charset = Charset.forName("UTF-8"); // encoding used for storing hash values as strings

    static final String hashName = "MD5"; // MD5 gives good enough accuracy in most circumstances. Change to SHA1 if it's needed
    static final ThreadLocal<MessageDigest> digestFunction = new ThreadLocal<MessageDigest>() {
        @Override
    ...
    public BloomFilter(int bitSetSize, int expectedNumberOfFilterElements, int actualNumberOfFilterElements, BitSet filterData) {
        ...
        this.numberOfAddedElements.set(actualNumberOfFilterElements);
    }
    ...
    public static int[] createHashes(byte[] data, int hashes) {
        ...
        int k = 0;
        byte salt = 0;
        MessageDigest tmp = digestFunction.get();
        while (k < hashes) {
            byte[] digest;
            {
                tmp.update(salt);
                salt++;
                digest = tmp.digest(data);
            }
    ...
    public boolean contains(byte[] bytes) {
        ...
        barrier = tmp+1;
        if (barrier == tmp) System.out.println("Strange things happen"); // I'm not sure about the compiler optimization.
        return true;
    }
     ...
     */
    public int count() {
        return (int)this.numberOfAddedElements.get();
    }
    ...
    public double getBitsPerElement() {
        return this.bitSetSize / (double)numberOfAddedElements.get();
    }
}
