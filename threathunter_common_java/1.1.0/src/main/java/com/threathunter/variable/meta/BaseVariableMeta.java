...
import com.threathunter.variable.IndexParser;
...
public abstract class BaseVariableMeta extends VariableMeta {
    ...
    protected NamedType valueSubType;
    ...
    public void setValueSubType(NamedType valueSubType) {
    ...
    public Property findPropertyByName(String name) {
        ...
        if (name.equals("value")) {
            return new Property(this.getId(), "value", NamedType.fromCode(getValueType()),
                    getValueSubType().isEmpty()? null : NamedType.fromCode(getValueSubType()));
        }
    ...
    public String getValueSubType() {
        return valueSubType == null ? "" : valueSubType.getCode();
    }
    ...
    public Object to_json_object() {
        ...
        if (getSrcEventMetaID() != null) {
            ...
            map.put("source", getSrcEventMetaID().getKeys().get(1));
            srcs.add(map);
        } else {
            getSrcVariableMetasID().forEach(id -> {
                Map<String, String> map = new HashMap<>();
                map.put("app", id.getKeys().get(0));
                map.put("source", id.getKeys().get(1));
                srcs.add(map);
            });
        }
    ...
    protected static <T extends BaseVariableMeta> T from_json_object(Object obj, T result) {
        ...
        result.parseFunction((Map<String, Object>) map.get("function"));

        result.setProperties(result.parseProperties());
        result.setValueType(result.parseValueType());
    ...
    protected abstract void parseFunction(Map<String, Object> functionJson);
    protected abstract List<Property> parseProperties();
    protected abstract NamedType parseValueType();
}
